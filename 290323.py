...
def risoluzione_quad(pippo, baudo):
    ...
    pippo = pippo[np.where(energie != 1.275)]
    baudo = baudo[np.where(energie != 1.275)]
    energie = energie[np.where(energie != 1.275)]
    ...
    chisq = ((((pippo - (ar*energie*energie + br*energie + cr))**2 / (ar*energie*energie + br*energie + cr)))).sum()
    print(f'Chisquare = {chisq:.1f}')
    chisq_norm=chisq/(len(pippo)-3)
    print(f'Chisquare norm = {chisq_norm:.1f}')
...
def dedA(franco):
    return ((4*A*(franco - C))/(np.sqrt(B**2 - 4*A*(C - franco))) + 2*B - 2*np.sqrt(B**2 - 4*A*(C - franco)))/(4*A*A)

def dedB(franco):
    return (-1)/(2*A) + B/(np.sqrt(B**2 - 4*A*(C - franco)))

def dedC(franco):
    return (-1)/(np.sqrt(B**2 - 4*A*(C - franco)))
    
def m(picco, energia, err_energia, angolo):
    me = picco*(1 - np.cos(angolo))/(picco/energia - 1)
    err_me = np.sqrt((err_energia*(picco*me)/(energia*(picco-energia)))**2 + (np.radians(0.5)*picco*(np.sin(angolo))/(picco/energia - 1))**2)
    return me, err_me
...
def dati_1cal(file, inf, med, sup, angolo):
    ...
    err_E1_sing_quad = np.sqrt((err_A*dedA(E1_sing))**2 + (err_B*dedB(E1_sing))**2 + (err_C*dedC(E1_sing))**2 + 2*dedA(E1_sing)*dedB(E1_sing)*correlazione_A_B + 2*dedA(E1_sing)*dedC(E1_sing)*correlazione_A_C + 2*dedB(E1_sing)*dedC(E1_sing)*correlazione_B_C)

    E2_sing_quad = (-B + np.sqrt(B**2 - 4*A*(C - E2_sing)))/(2*A)
    err_E2_sing_quad = np.sqrt((err_A*dedA(E2_sing))**2 + (err_B*dedB(E2_sing))**2 + (err_C*dedC(E2_sing))**2 + 2*dedA(E2_sing)*dedB(E2_sing)*correlazione_A_B + 2*dedA(E2_sing)*dedC(E2_sing)*correlazione_A_C + 2*dedB(E2_sing)*dedC(E2_sing)*correlazione_B_C)
    ...
    err_E1_multi_quad = np.sqrt((err_A*dedA(E1_multi))**2 + (err_B*dedB(E1_multi))**2 + (err_C*dedC(E1_multi))**2 + 2*dedA(E1_multi)*dedB(E1_multi)*correlazione_A_B + 2*dedA(E1_multi)*dedC(E1_multi)*correlazione_A_C + 2*dedB(E1_multi)*dedC(E1_multi)*correlazione_B_C)

    E2_multi_quad = (-B + np.sqrt(B**2 - 4*A*(C - E2_multi)))/(2*A)
    err_E2_multi_quad = np.sqrt((err_A*dedA(E2_multi))**2 + (err_B*dedB(E2_multi))**2 + (err_C*dedC(E2_multi))**2 + 2*dedA(E2_multi)*dedB(E2_multi)*correlazione_A_B + 2*dedA(E2_multi)*dedC(E2_multi)*correlazione_A_C + 2*dedB(E2_multi)*dedC(E2_multi)*correlazione_B_C)
...
file = [r"C:Data\290323\290323_americio2.dat",
        r"C:Data\290323\290323_sodio2.dat",
        r"C:Data\290323\290323_cesio2.dat",
        r"C:Data\290323\290323_cobalto2.dat",
        r"C:Data\290323\290323_sodio2.dat",
        r"C:Data\290323\290323_cobalto2.dat"]
...
dati_1cal(r"C:Data\290323\290323_test6(20gradi).dat", 4612, 5349, 5866, theta)